...
public class RemoveDuplicates2Medium {
    public int removeDuplicates(int[] nums) {
        int first = 1;
        int nextUnique = 1;
        boolean sameCopied = false;
        // 3 cases -> not same copy, same check once same copied, else move nextUnique to search nextUnique
        while(nextUnique<nums.length){
            if(nums[nextUnique]!=nums[first-1]){
                nums[first] = nums[nextUnique];
                first++;
                nextUnique++;
                sameCopied = false;
            }
            else if(nums[nextUnique] == nums[first-1] && !sameCopied){
                nums[first] = nums[nextUnique];
                sameCopied = true;
                first++;
                nextUnique++;
