...
public class MinDaysToMakeMBouq {
    public int minDays(int[] bloomDay, int m, int k) {
        int left = Integer.MAX_VALUE;
        int right = Integer.MIN_VALUE;
        for(int i=0;i<bloomDay.length;i++){
            left = Integer.min(left, bloomDay[i]);
            right = Integer.max(right, bloomDay[i]);
        }
        int minDays = -1;
        while(left<=right){
            int mid = (left+right)/2;
            if(findNB(bloomDay, mid, k)>=m){
                minDays = mid;
                right = mid - 1;
            }
            else{
    ...
    private static int findNB(int[] bloomDay, int num, int k){
        int count = 0;
        int bouq = 0;
        for(int i=0;i<bloomDay.length;i++){
            if(bloomDay[i]<=num){
