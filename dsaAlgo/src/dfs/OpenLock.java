    ...
    public int openLock(String[] deadends, String target) {
        ...
        while(!q.isEmpty()){
            ...
            for(int i=0;i<ch.length;i++){
                char orChar = ch[i];
                ch[i] = upChar(orChar);
                String updated = new String(ch);
                if(!visited.contains(updated) && !dead.contains(updated)){
                    visited.add(updated);
                    q.offer(new Pair(new String(ch), dis+1));
                }
                ch[i] = downChar(orChar);
                String updatedd = new String(ch);
                if(!visited.contains(updatedd) && !dead.contains(updatedd)){
                    visited.add(updatedd);
                    q.offer(new Pair(new String(ch), dis+1));
                }
                ch[i] = orChar;
            }
